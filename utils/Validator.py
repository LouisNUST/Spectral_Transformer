...
class Validator(object):
    ...
    def __init__(self, timeStr,valid_img_dir, valid_lab_dir, valid_result_dir, valid_log_dir, best_model_dir,
        ...
        self.valid_log_dir = valid_log_dir + "/"+datasetName+'_'+timeStr+"_valid.txt" # 验证集测试指标的路径
    ...
    def validate(self, epoch_num):
        ...
        with open(self.valid_log_dir, 'a', encoding='utf-8') as fout:
            line = time.strftime('%m-%d %X')+" epoch:{}  | ODS:{:.6f} | OIS:{:.6f} | max ODS:{:.6f} | max OIS:{:.6f} " \
